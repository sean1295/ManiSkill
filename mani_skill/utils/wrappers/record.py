        ...
        if self.save_video:
            ...
            if self.info_on_video:
                ...
                if isinstance(rew, torch.Tensor) and len(rew.shape) > 1:
                    _rew = float(common.to_numpy(rew[0]))
                else:
                    _rew = float(common.to_numpy(rew))
                    
                # _action = action[0] if len(action.shape) == 2 else action
                extra_texts = [
                    f"reward: {_rew:.3f}",
                    # "action: {}".format(",".join([f"{x.item():.2f}" for x in _action])),
                ]
